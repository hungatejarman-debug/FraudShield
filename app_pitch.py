...
with tab1:
    ...
    with left:
        st.markdown(
            """
            <div class="panel">
                <div class="panel-title">路演讲解重点</div>
                <div class="panel-text">
                第一步先向评委说明：系统不是人工一笔笔检查交易，
                而是先对全部交易自动评分，再把中高风险交易筛选出来。
                这样可以把风控人员的注意力集中到真正需要关注的案例上。
                <br><br>
                当前演示数据中，系统已完成全部交易的风险分级，
                并将交易划分为正常、低风险、中风险和高风险四类。
                </div>
            </div>
            """,
            unsafe_allow_html=True
    ...
    if global_df.empty:
        ...
        with right:
            ...
            st.markdown(
                f"""
                <div class="panel">
                    <div class="panel-title">评委应该听到的重点</div>
                    <div class="panel-text">
...
with tab5:
    ...
    with right:
        st.markdown(
            ...
            各位评委老师好，我们的项目是 FraudShield 金融交易反诈智能辅助系统。
            它面向银行、支付机构和消费金融平台的交易反诈场景，目标是解决传统风控中
            “人工审核压力大、模型判断不透明、客户提醒生硬”三个问题。
            <br><br>

            首先，系统会对交易数据进行自动风险评分，并将交易划分为正常、低风险、中风险和高风险。
            这样风控人员不需要逐笔查看全部交易，而是可以优先关注中高风险交易。
            <br><br>

            其次，我们不仅输出风险分数，还引入 SHAP 模型解释方法，说明模型为什么认为某笔交易存在风险。
            例如，系统会关注交易类型、交易金额、付款方交易后余额、账户余额变化等特征。
            这让模型从黑箱判断变成可解释的辅助决策。
            <br><br>

            第三，系统会针对不同对象生成不同文本。面向风控人员，生成结构化审核解释和处置建议；
            面向客户，生成温和、清晰的风险提醒，引导客户核实收款方身份，避免因生硬拦截影响体验。
            <br><br>

            当前系统已经完成数据读取、模型训练、风险预测、模型解释和可视化展示。
            后续可以部署到云服务器或金融机构内网，通过接口接入真实交易流水，
            形成可推广的智能反诈辅助服务。
            </div>
            """,
            unsafe_allow_html=True
